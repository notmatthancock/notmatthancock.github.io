...
This is sometimes called the "scatter matrix". If we divide it by $n$, it is the [maximum likelihood of the covariance matrix](https://en.wikipedia.org/wiki/Estimation_of_covariance_matrices) assuming the $\mathbf{x}_j$ are zero mean i.i.d. multivariate gaussian. I will assume that $\text{rank}(S) = p$, which is reasonable for $n \gt \gt p$.

## The best 1D subspace for the data

Ok then. So we have all these data points. For the sake of visualization, we take $p=2$ as shown below.

<div style="text-align: center;"><img src="{{ site.baseurl }}/images/pca-1.png" width="50%"/></div>

Now, here's the question: given a point set, what is the best single direction to project all the data onto? We could project the data onto a higher dimensional subspace (some hyperplane), but for now just consider projecting all the data onto a line. Recall that the projection of a vector, $\mathbf{x}$, onto the vector $\mathbf{w}$ is given by:
...
$$
  ||\mathbf{x} - \frac{\mathbf{w}\mathbf{w}^T}{\mathbf{w}^T\mathbf{w}}\mathbf{x}||^2
$$
...
Thus, we seek a single direction, $\mathbf{w}$, which satisifies:

$$
    \mathbf{w} = \text{argmin} \sum_{i=1}^n ||\mathbf{x}_i - \frac{\mathbf{w}\mathbf{w}^T}{\mathbf{w}^T\mathbf{w}}\mathbf{x}_i||^2
$$

Let's call the function we're minimizing $f(\mathbf{w})$. So:

$$
    f(\mathbf{w}) = \sum_i \mathbf{x}_i^T\mathbf{x}_i - \frac{(\mathbf{x}_i^T\mathbf{w})^2}{\mathbf{w}^T\mathbf{w}}
$$

Writing the second term a little differently reveals what must be satisfied:
...
You might recognize the second term as the Rayleigh quotient for eigvenvalues of $S$. Thus to minimize this function, we should should $\mathbf{w}$ to be the eigenvector of $S$ corresponding the largest eigenvalue of $S$. 
...
$$
    \sum_i \mathbf{x}_i^T\mathbf{x}_i = \text{trace}(XX^T) = \text{trace}(X^TX) = \text{trace} = \sum_j \lambda_j
$$
...
is sometimes referred to as the "variance retained".
...
Of course, we rarely are fortunate enough to be able to project reduce our data to a subspace of a single dimension. Usually, we project onto the first $k$ of $p$ eigenvectors of $S$. The relative squared error is:

$$
    \frac{ \sum_{j=k+1}^{p} \lambda_j } { \sum_{j=1}^{p} \lambda_j }
$$

The derivation for projecting onto higher dimensions follows analogous to the previous case. I derive it here for completeness, but there's not really more going on except for a few required tricks. Let $W_k \in \mathbb{R}^{p \times k}$ denote the subspace of dimension $k$ that we project our data onto, minimizing the sum of squares error. That is,

$$
    \begin{align*}
        W_k &= \text{argmin} \sum_i ||\mathbf{x}_i - W_k(W_k^TW_k)^{-1}W_k^T\mathbf{x}_i||^2 \\
        &= \text{argmin} \sum_i \mathbf{x}_i^T\mathbf{x}_i - \mathbf{x}_i^TW_k(W_k^TW_k)^{-1}W_k^T\mathbf{x}_i
    \end{align*}
$$
